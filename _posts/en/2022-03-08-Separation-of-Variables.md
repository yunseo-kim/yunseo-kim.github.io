...
Initially, a tank contains 1000L of water with 10kg of salt dissolved. Salt water flows in at a rate of 10L per minute, with 0.2kg of salt dissolved per liter. The solution in the tank is well-stirred and maintained uniformly, and this salt water flows out at a rate of 10L per minute. Find the amount of salt $y(t)$ in the tank at time $t$.

### 1. Model Setup

$$ y'=\text{rate in} - \text{rate out}. $$

The salt inflow rate is 2kg per minute. The salt water outflow per minute is 0.01 of the total salt water capacity, so the salt outflow per minute is $0.01 y(t)$. Therefore, the model is the ordinary differential equation

$$y'=2-0.01y=-0.01(y-200) $$

### 2. Model Solution
The ordinary differential equation we set up earlier is separable. Let's separate variables, integrate, and then take the exponential of both sides.
...
Initially, the amount of salt in the tank is 10kg, so the initial condition is $y(0)=10$. Substituting $y=10,\ t=0$ into the above equation gives $10-200=ce^0=c$, so $c=-190$.
...
Let's choose 10 PM as $t=0$, then the given initial condition is $T(0)=20$. Let's call the particular solution we get at this time $T_p$. Substituting:
...
Since the internal building temperature was 17.4℃ at 2 AM, $T(4)=17.4$. If we algebraically find the value of $k$ and put $k$ into $T_p(t)$:
...
where $\Delta h(>0)$ is the decrease in water height $h(t)$. If we set $\Delta V$ and $\Delta V^*$ equal, we get
...
In some cases, it's possible to transform a non-separable ordinary differential equation into a separable one by introducing a new unknown function for $y$.

$$ y'=f\left(\frac {y}{x}\right). $$

When solving such an ordinary differential equation, if we let $y/x=u$, then
