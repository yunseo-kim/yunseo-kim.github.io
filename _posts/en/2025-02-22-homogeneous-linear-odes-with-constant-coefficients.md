...
Previously, in the [Bernoulli Equation](/posts/Bernoulli-Equation/), we found the general solution for the logistic equation. According to that, the solution to a first-order linear ODE with constant coefficient $k$:
...
> Of course, this is just a guess, and there's no guarantee that the general solution will actually take this form. However, as long as we can find two linearly independent solutions, we can obtain the general solution using the [superposition principle](/posts/homogeneous-linear-odes-of-second-order/#superposition-principle), as we saw in [Second-Order Homogeneous Linear ODEs](/posts/homogeneous-linear-odes-of-second-order/#basis-and-general-solution).  
> As we'll see shortly, there are [cases where we need to find solutions in other forms](#ii-real-double-root-lambda---cfraca2).
{: .prompt-info }
...
In this situation, we can use the [reduction of order](/posts/homogeneous-linear-odes-of-second-order/#reduction-of-order) method that we learned earlier. Let's assume the second solution we're looking for is of the form $y_2=uy_1$, and substitute
...
By the [superposition principle](/posts/homogeneous-linear-odes-of-second-order/#superposition-principle), the sum and constant multiples of these complex solutions are also solutions. Therefore, by adding these two equations and multiplying both sides by $\cfrac{1}{2}$, we can obtain the first real solution $y_1$ as follows:
