...
Es uno de los conceptos principales y una premisa básica de la física, que establece que todas las leyes físicas deben ser idénticas en diferentes sistemas de referencia que se mueven a velocidad constante entre sí. Si los observadores en movimiento relativo experimentaran leyes físicas diferentes, esta diferencia podría usarse para establecer un sistema de referencia absoluto y determinar quién está en reposo y quién en movimiento. Sin embargo, según el principio de relatividad, tal distinción no existe, por lo que no hay un sistema de referencia absoluto o movimiento absoluto para todo el universo, y todos los sistemas de referencia inerciales son equivalentes.
...
1. $$\begin{gather*}\nabla\cdot{E}=\frac{q}{\epsilon_0} \\
 \text{: El flujo eléctrico a través de cualquier superficie cerrada es igual a la carga neta dentro de ella (Ley de Gauss).}
 \end{gather*}$$
2. $$\begin{gather*}\nabla\cdot{B}=0 \\
...
- Aunque la velocidad de la luz $c$ se puede calcular a partir de las ecuaciones de Maxwell y es una constante invariable, según la mecánica newtoniana y la transformación de Galileo, la velocidad de la luz $c$ se mediría de manera diferente en diferentes sistemas inerciales.

Por lo tanto, las ecuaciones de Maxwell y la transformación de Galileo son incompatibles entre sí, y al menos una de ellas debía ser modificada. Esto se convirtió en el trasfondo para la aparición de la **transformación de Lorentz (Lorentz transformation)** que se discutirá más adelante.
...
En este experimento, el rayo de luz se divide en dos al pasar por un espejo semitransparente, luego cada rayo recorre ida y vuelta los dos brazos perpendiculares del interferómetro, recorriendo un total de unos 11 m, y se encuentran en el punto medio, donde se produce un patrón de interferencia constructiva o destructiva según la diferencia de fase entre los dos rayos. Según la teoría del éter, la velocidad de la luz debería variar dependiendo de la velocidad relativa al éter, por lo que se esperaba observar un cambio en este patrón de interferencia, pero en realidad no se pudo observar ningún cambio. Hubo varios intentos de explicar este resultado experimental, entre los cuales FitzGerald y Lorentz propusieron la *contracción de Lorentz-FitzGerald (Lorentz–FitzGerald contraction)* o *contracción de longitud (length contraction)*, que sugiere que un objeto se contrae en longitud cuando <u>se mueve relativamente al éter</u>, lo cual conduce a la transformación de Lorentz.

> En ese momento, Lorentz creía en la existencia del éter y pensaba que la contracción de longitud ocurría debido al movimiento relativo al éter. Posteriormente, Einstein interpretó el verdadero significado físico de la transformación de Lorentz con su *Teoría de la Relatividad Especial (Theory of Special Relativity)*, explicando la contracción de longitud en términos del concepto de espacio-tiempo en lugar del éter, y más tarde se demostró que el éter no existe.
{: .prompt-info }
...
- Como se sabe que la transformación de Galileo es correcta en situaciones mecánicas cotidianas, debe poder aproximarse a la ecuación ($\ref{eqn:galilean_transform_x}$).
...
Sustituyendo $x$ y $t$ en esta ecuación usando las ecuaciones ($\ref{eqn:lorentz_transform_x}$) y ($\ref{eqn:lorentz_transform_t}$):
...
Sin embargo, como vimos en la ecuación ($\ref{eqn:ct_S}$), $x=ct$, por lo que:
...
Las ecuaciones de transformación que hemos obtenido finalmente son las siguientes:
...
Lorentz demostró que al usar estas ecuaciones de transformación, las fórmulas fundamentales del electromagnetismo se mantienen en la misma forma en todos los sistemas de referencia inerciales. Además, podemos ver que cuando la velocidad $v$ es muy pequeña en comparación con la velocidad de la luz $c$, $\gamma \to 1$, por lo que se puede aproximar a la transformación de Galileo.
