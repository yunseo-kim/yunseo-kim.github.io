...
description: >-
  Découvrons les caractéristiques et les différences entre les pages web statiques et dynamiques, les générateurs de sites statiques (Static Site Generator), et hébergeons un blog Jekyll sur GitHub Pages.
categories:
  - Blogging
tags:
  - Jekyll
...
~~En réalité, c'est surtout parce que je ne suis pas encore familier avec l'hébergement de sites statiques.~~
...
#### Pages web statiques (Static Web Page)
...
#### Pages web dynamiques (Dynamic Web Page)
...
- Selon la structure de la page web, les utilisateurs peuvent ajouter, modifier et supprimer des données depuis le navigateur
...
- Après avoir créé un dépôt nommé au format '{username}.github.io' correspondant à votre nom d'utilisateur GitHub, vous pouvez soit pousser directement les pages HTML construites dans ce dépôt, soit utiliser GitHub Actions pour effectuer la construction et le déploiement
...
- Est-il possible de minimiser les essais et erreurs inutiles et de se concentrer sur la rédaction d'articles et la gestion du blog ?
...
- Y a-t-il une variété de thèmes et de plugins disponibles ?
  - Même si on utilise un SSG plutôt que d'écrire directement du HTML, créer soi-même divers modèles est fastidieux et prend du temps, et ce n'est pas vraiment nécessaire. Il y a déjà beaucoup d'excellents thèmes disponibles en ligne, il suffit d'en adopter un qui vous plaît et de l'utiliser.
  ...
  - Avec Jekyll, j'ai pu rapidement trouver un thème qui me plaisait au premier coup d'œil, alors qu'avec Hugo ou Gatsby, il ne semblait pas y avoir autant de thèmes adaptés à un blog personnel. Comme mentionné précédemment, la compatibilité avec GitHub Pages, que de nombreux développeurs utilisent pour héberger des blogs personnels, et la période de développement semblent avoir eu un impact important ici aussi.
...
- Prise en charge de base de Google Analytics
- Possibilité de choisir parmi des skins prédéfinis
- Bien que j'aie ensuite découvert le thème Chirpy qui a un design plus élégant et qui me plaît davantage, je pense que c'était un thème assez correct à utiliser, avec un design plutôt épuré même s'il n'était pas particulièrement joli, compte tenu du fait qu'il s'agit d'un blog d'ingénierie après tout.

### Thème Jekyll Chirpy (depuis avril 2022)
...
- Prise en charge de base de l'expression des équations en syntaxe LaTeX basée sur MathJax
- Prise en charge de base des fonctions de diagramme basées sur Mermaid
...
- Surtout, le design est joli. Le thème Minimal Mistakes est épuré mais a une certaine rigidité qui semble plus adaptée à une documentation technique officielle de projet ou à une page de portfolio qu'à un blog, tandis que le thème Chirpy a l'avantage d'avoir un design qui ne fait pas pâle figure même comparé aux plateformes de blogs commerciales comme Tistory, Medium ou velog.

## 3. Création du dépôt GitHub, construction et déploiement
Je décris ici le processus basé sur le thème Jekyll Chirpy actuellement utilisé (juin 2024), en supposant que Git est déjà installé.  
Référez-vous au [guide d'installation officiel de Jekyll](https://jekyllrb.com/docs/installation/) et à la [page officielle du thème Jekyll Chirpy](https://github.com/cotes2020/jekyll-theme-chirpy/wiki).
...
La [page officielle du thème Jekyll Chirpy](https://chirpy.cotes.page/posts/getting-started/#creating-a-new-site) présente les deux méthodes suivantes :
1. Méthode utilisant le gem "jekyll-theme-chirpy" pour importer les fichiers principaux et le reste des ressources à partir du modèle [Chirpy Starter](https://github.com/cotes2020/chirpy-starter)
  - Avantage : L'application des mises à niveau de version est facile, comme nous le verrons plus tard.
...
J'ai choisi la méthode 1. Dans le cas du thème Chirpy, il est déjà très complet, donc la plupart des utilisateurs n'ont pas grand-chose à personnaliser, et comme il est encore activement développé et amélioré jusqu'en 2024, les avantages de suivre l'upstream original en temps voulu l'emportent sur les avantages d'appliquer sa propre personnalisation, à moins de faire des modifications majeures. Le guide officiel du thème Chirpy recommande également la méthode 1 pour la plupart des utilisateurs.

### 3-3. Configurations principales
Appliquez les configurations nécessaires dans les fichiers `_config.yml`{: .filepath} du répertoire racine et `_data/contact.yml`{: .filepath}, `_data/share.yml`{: .filepath}. Les commentaires sont bien écrits et les configurations sont intuitives, donc vous pouvez les appliquer sans difficulté particulière. Les seules configurations qui nécessitent un travail séparé à l'extérieur sont l'enregistrement du code d'authentification pour l'intégration de Google Search Console et l'intégration d'outils de webmaster comme Google Analytics ou GoatCounter, mais ce n'est pas vraiment une procédure compliquée et ce n'est pas le sujet principal que je veux aborder dans cet article, donc je vais omettre les détails.
...
2. Construction et déploiement manuels (dans le cas d'utilisation d'un autre service d'hébergement ou d'auto-hébergement)
...
Le [guide de rédaction d'articles](https://chirpy.cotes.page/posts/write-a-new-post/) du thème Chirpy documente bien la méthode de rédaction d'articles et les options disponibles. Il offre diverses fonctionnalités en plus de celles décrites dans cet article, et c'est un contenu utile à consulter si nécessaire. Ici, je vais résumer les points principaux à garder à l'esprit à chaque fois que vous publiez un article.
...
Vous devez rédiger correctement le Front Matter au début