...
title: "放射平衡の計算"
description: >-
  放射性核種の崩壊定数と半減期、平均寿命の相関関係について学び、与えられた崩壊連鎖における任意の時間tでの放射性核種の放射能を計算する。
...
## 例題：放射性崩壊連鎖 1
ある放射性核種が $R$ atom/s の速度で生成されると仮定しよう。この核は生成されるとすぐに放射性崩壊が起こる。任意の時刻 t でのこの核種の放射能を求めよ。
```mermaid
flowchart LR
	Start[?] -- R --> A[数学的モデル]
	A -- α --> End[?]
...
$$ \text{時間に伴う核種の変化率} = \text{生成率}-\text{損失率} $$
...
### 2. 一般解
$n$ に関する項をすべて左辺に移項し、両辺に $e^{\lambda t}$ をかける。
...
### 3. 特殊解
$t=0$ のときにこの核種の数が $n_0$ だとして定数 $c$ の値を求める。
...
である。上の式の両辺に $\lambda$ をかけてこの核種の放射能を求めることができる。
...
### 2. 一般解
微分方程式を解くために $n_B$ に関する項をすべて左辺に移項し、両辺に $e^{\lambda_B t}$ をかける。
...
### 3. 特殊解
$t=0$ のとき B 元素の数が $n_{B0}$ だとして定数 $c$ の値を求める。
