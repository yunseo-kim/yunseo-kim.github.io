...
description: マークダウンテキストファイルの多言語翻訳用のプロンプトをデザインし、Anthropicから発行されたAPIキーと作成したプロンプトを適用してPythonで作業を自動化するプロセスを扱う。このポストはシリーズの2番目の記事で、API発行と連携、Pythonスクリプト作成方法を紹介する。
