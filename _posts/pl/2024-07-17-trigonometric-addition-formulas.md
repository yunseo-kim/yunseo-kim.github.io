---
title: Wzory na sumę i różnicę funkcji trygonometrycznych
description: Omawiamy definicje funkcji trygonometrycznych i zależności między nimi, a następnie wyprowadzamy wzory na sumę i różnicę oraz wzory pochodne.
categories: [Mathematics, Trigonometry]
tags: [Trigonometric Addition Formulas]
math: true
image: /assets/img/math-and-physics-cropped.webp
---
## Definicja funkcji trygonometrycznych
![Unit Circle and Radius Vector](/assets/img/trigonometry/definition.png)
Jak na rysunku, niech półprosta (promień wodzący) $\overline{OP}$ tworzy z dodatnim zwrotem osi $x$ kąt o mierze $\theta$. Wówczas definiujemy
$$
\begin{gather}\begin{split}
\sin \theta = \frac { y } { r } ,\; \cos \theta = \frac { x } { r } ,\; \tan \theta = \frac { y } { x } \\ \csc \theta = \frac { r } { y } ,\; \sec \theta = \frac { r } { x } ,\; \cot \theta = \frac { x } { y } \end{split}\end{gather}
$$
i łącznie nazywamy je funkcjami trygonometrycznymi.

## Zależności między funkcjami trygonometrycznymi
1. $$ \csc \theta = \frac { 1 } { \sin \theta } ,\; \sec \theta = \frac { 1 } { \cos \theta } ,\; \cot \theta = \frac { 1 } { \tan \theta } \tag{2}$$
2. $$ \tan \theta = \frac { \sin \theta } { \cos \theta } ,\; \cot \theta = \frac { \cos \theta } { \sin \theta } \tag{3}$$
3. $$ \tag{4} \begin{gather*}
\sin ^ { 2 } \theta + \cos ^ { 2 } \theta = 1 \\
\tan ^ { 2 } \theta + 1 = \sec ^ { 2 } \theta \\
1 + \cot ^ { 2 } \theta = \csc ^ { 2 } \theta 
\end{gather*} 
$$

## Wzory na sumę i różnicę funkcji trygonometrycznych
![Deriving the Trigonometric Addition Formulas](/assets/img/trigonometry/trigonometric-addition-formulas.png)
Jak na rysunku, niech cztery półproste, które tworzą z dodatnim zwrotem osi $x$ kąty o miarach odpowiednio $\alpha+\beta,\, 0,\, \alpha,\, -\beta$, przecinają okrąg jednostkowy w punktach odpowiednio $P, Q, P^{\prime}, Q^{\prime}$.

Współrzędne punktów $P, Q$ są równe
$$
P(\cos(\alpha+\beta), \sin(\alpha+\beta)),\; Q(1,0)
$$
zatem
$$
\begin{align*} \overline { P^ { \prime } Q^ { \prime } } ^2&= \{ \cos \alpha - \cos ( - \beta ) \} ^ { 2 } + \{ \sin \alpha - \sin ( - \beta ) \} ^ { 2 } \\
&= 2 - 2 \cos \alpha \cos ( - \beta ) - 2 \sin \alpha \sin ( - \beta ) \\
&= 2 - 2 \cos \alpha \cos \beta + 2 \sin \alpha \sin \beta. \end{align*}
$$

Ponieważ $\overline{PQ}=\overline{P^{\prime} Q^{\prime}}$, mamy
$2 - 2 \cos ( \alpha + \beta ) = 2 - 2 \cos \alpha \cos \beta + 2 \sin \alpha \sin \beta.$

$$
 \therefore \cos ( \alpha + \beta ) = \cos \alpha \cos \beta - \sin \alpha \sin \beta. \label{eqn:cos_1} \tag{5}
$$

Podstawiając w powyższym wzorze $-\beta$ w miejsce $\beta$ i upraszczając, otrzymujemy
$$
\cos ( \alpha - \beta ) = \cos \alpha \cos \beta + \sin \alpha \sin \beta \label{eqn:cos_2} \tag{6}
$$

Ponieważ $\cos ( \frac { \pi } { 2 } - \theta ) = \sin \theta ,\, \sin ( \frac { \pi } { 2 } - \theta ) = \cos \theta$, to
$$
\begin{align*} \sin ( \alpha + \beta ) &= \cos ( \frac { \pi } { 2 } - ( \alpha + \beta ) ) = \cos ( ( \frac { \pi } { 2 } - \alpha ) - \beta) \\ &= \cos ( \frac { \pi } { 2 } - x ) \cos \beta + \sin ( \frac { \pi } { 2 } - \alpha ) \sin \beta \\ &= \sin \alpha \cos \beta + \cos \alpha \sin \beta. \end{align*}
$$

$$
\therefore \sin ( \alpha + \beta ) = \sin \alpha \cos \beta + \cos \alpha \sin \beta. \label{eqn:sin_1} \tag{7}
$$

Podstawiając w tym wzorze $-\beta$ w miejsce $\beta$ i upraszczając, dostajemy
$$
\sin ( \alpha - \beta ) = \sin \alpha \cos \beta - \cos \alpha \sin \beta. \label{eqn:sin_2} \tag{8}
$$

Na koniec, z
$$
\tan ( \alpha + \beta ) = \frac { \sin ( \alpha + \beta ) } { \cos ( \alpha + \beta ) } = \frac { \sin \alpha \cos \beta + \cos \alpha \sin \beta } { \cos \alpha \cos \beta - \sin \alpha \sin \beta }
$$
dzieląc licznik i mianownik przez $\cos{\alpha} \cos{\beta}$ i upraszczając, otrzymujemy
$$
\tan ( \alpha + \beta ) = \frac { \tan \alpha + \tan \beta } { 1 - \tan \alpha \tan \beta } \label{eqn:tan_1} \tag{9}
$$
a podstawiając $-\beta$ w miejsce $\beta$ i upraszczając,
$$
\tan ( \alpha - \beta ) = \frac { \tan \alpha - \tan \beta } { 1 + \tan \alpha \tan \beta } \label{eqn:tan_2} \tag{10}
$$
co należało wykazać.

## Miara kąta ostrego między dwiema prostymi o zadanych nachyleniach
![Angle formed by two lines](/assets/img/trigonometry/angle-formed-by-two-lines.png)
Korzystając ze wzoru ($\ref{eqn:tan_2}$), można obliczyć miarę kąta ostrego między dwiema prostymi o zadanych współczynnikach kierunkowych. Jeśli proste $y=mx+b$, $y=m^{\prime} x+b^{\prime}$ tworzą z dodatnim zwrotem osi $x$ kąty o miarach odpowiednio $\theta_{1}$, $\theta_{2}$, to
$$
\tan{\theta_{1}}=m,\, \tan{\theta_{2}}=m^{\prime}
$$
Zatem, jeśli miarę kąta ostrego między tymi prostymi oznaczymy przez $\theta$, to
$$
\tag{11} \begin{align*}
\tan{\theta}&=\left\vert \tan{\theta_{2}-\theta_{1}} \right\vert=\left\vert \frac{\tan{\theta_2}-\tan{\theta_1}}{1+\tan{\theta_1}\tan{\theta_2}}\right\vert \\
&=\left\vert \frac{m-m^{\prime}}{1+mm^{\prime}} \right\vert.
\end{align*}
$$
