...
description: >-
  Aprenda sobre o conceito de sistemas de referência e a transformação de Galileu, amplamente utilizada na mecânica clássica. Também, examine brevemente as equações de Maxwell e o experimento de Michelson-Morley, que serviram de base para o surgimento da transformação de Lorentz, e derive a matriz de transformação de Lorentz.
categories: [Física de Engenharia, Física Moderna]
tags: [Teoria da Relatividade, Transformação Linear, Transformação de Lorentz]
math: true
---

## Sistemas de Referência e Princípio da Relatividade
### Sistema de Referência (frame of reference)
- **Sistema de Referência (frame of reference)**: Dizer que um objeto se move significa que sua posição muda em relação a outros objetos. Como todo movimento é relativo, para descrever qualquer movimento, é necessário estabelecer um sistema de referência como base.
...
Um dos principais conceitos e premissas fundamentais da física, que afirma que todas as leis físicas devem ser as mesmas em todos os sistemas de referência que se movem com velocidade constante entre si. Se observadores em movimento relativo experimentassem leis físicas diferentes, essa diferença poderia ser usada para estabelecer um sistema de referência absoluto e determinar quem está parado e quem está se movendo. No entanto, de acordo com o princípio da relatividade, tal distinção não existe, portanto, não há sistema de referência absoluto ou movimento absoluto em relação ao universo inteiro, e todos os sistemas de referência inerciais são equivalentes.

## Limitações da Transformação de Galileu
### Transformação de Galileu (Galilean transformation)
Suponha que existam dois sistemas inerciais $S$ e $S^{\prime}$, onde $S^{\prime}$ se move em relação a $S$ com uma velocidade constante $\vec{v}$ na direção $+x$, e um mesmo evento é observado no sistema $S$ na coordenada $(x, y, z)$ no tempo $t$, e no sistema $S^{\prime}$ na coordenada $(x^{\prime}, y^{\prime}, z^{\prime})$ no tempo $t^{\prime}$.
...
As equações ($\ref{eqn:galilean_transform_x}$) a ($\ref{eqn:galilean_transform_t}$) acima são chamadas de **transformação de Galileu (Galilean transformation)**, que é a transformação de coordenadas entre diferentes sistemas inerciais classicamente usada na física. É simples e intuitiva, e funciona bem na maioria das situações cotidianas. No entanto, como será discutido mais adiante, ela entra em contradição com as equações de Maxwell.

### Equações de Maxwell
No final do século XIX, Maxwell (Maxwell) expandiu as ideias e resultados de pesquisas anteriores propostos por outros cientistas como Faraday (Faraday) e Ampère (Ampere), revelando que a eletricidade e o magnetismo são, na verdade, uma única força, e derivou as seguintes quatro equações que descrevem o campo eletromagnético:
...
As equações de Maxwell conseguiram explicar com sucesso todos os fenômenos elétricos e magnéticos conhecidos até então, previram a existência de ondas eletromagnéticas e também deduziram que a velocidade $c$ das ondas eletromagnéticas no vácuo é uma constante invariável, tornando-se assim a fórmula central do eletromagnetismo.

### Contradição entre a Transformação de Galileu e as Equações de Maxwell
A mecânica newtoniana, que utiliza a transformação de Galileu, foi a base da física por mais de 200 anos, e as equações de Maxwell, como mencionado acima, são as equações centrais que descrevem os fenômenos elétricos e magnéticos. No entanto, existe uma contradição entre as duas:
...
- As equações de Maxwell permitem calcular a magnitude da velocidade da luz $c$, que é uma constante invariável, mas de acordo com a mecânica newtoniana e a transformação de Galileu, a velocidade da luz $c$ é medida diferentemente em diferentes sistemas inerciais.
...
De acordo com a teoria do éter, mesmo que o espaço cósmico fosse vácuo, estaria preenchido com éter, então acreditava-se que o movimento orbital da Terra, que se move a uma velocidade de cerca de 30km/s em relação ao Sol, formaria um vento de éter atravessando a Terra.
...
No entanto, da equação ($\ref{eqn:ct_S}$) anterior, sabemos que $x=ct$, então:
...
Lorentz (Lorentz) mostrou que, usando estas equações de transformação, as fórmulas fundamentais do eletromagnetismo têm a mesma forma em todos os sistemas de referência inerciais. Além disso, podemos ver que quando a velocidade $v$ é muito menor que a velocidade da luz $c$, $\gamma \to 1$, então pode ser aproximada pela transformação de Galileu.
