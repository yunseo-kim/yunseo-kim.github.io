...
最近我引入了Anthropic的Claude 3.5 Sonnet API來進行博客文章的多語言翻譯。在這個系列中,我將討論選擇Claude 3.5 Sonnet API的原因、提示設計方法,以及如何通過Python腳本實現API集成和自動化。

該系列由兩篇文章組成,您正在閱讀的是該系列的第一篇。
- 第1部分:介紹Claude 3.5 Sonnet模型及選擇理由,提示工程(本文)  
...
即使不使用Claude 3.5或GPT-4等語言模型,也存在Google翻譯或DeepL等現有商業翻譯API。儘管如此,我決定使用LLM進行翻譯的原因是,與其他商業翻譯服務不同,用戶可以通過提示設計向模型提供寫作目的或主要主題等額外的上下文信息或要求,而模型可以根據這些信息提供考慮上下文的翻譯。雖然DeepL或Google翻譯通常也提供出色的翻譯質量,但由於無法很好地把握文章主題或整體上下文,在要求翻譯非日常對話的專業主題長文時,翻譯結果相對來說可能會顯得不自然。特別是如上所述,Claude在寫作、語言推理、多語言理解和翻譯領域被認為比競爭對手GPT-4相對更出色,我自己進行簡單測試時也發現它比GPT-4提供更流暢的翻譯質量,因此我認為它適合翻譯本博客上的工程相關文章。

## 提示設計

### 提出請求時的基本原則

為了從語言模型獲得符合目的的滿意結果,需要提供適當的提示。提示設計可能看起來令人望而生畏,但實際上,無論對方是語言模型還是人類,"如何有效提出請求"的方法並沒有太大區別,從這個角度來看就不難了。根據5W1H原則清楚地解釋當前情況和請求,必要時添加一些具體示例也很有幫助。雖然存在許多關於提示設計的技巧和方法,但大多數都源自於下面將要討論的基本原則。

#### 整體語氣

有許多報告表明,使用禮貌請求的語氣而不是高壓命令的語氣來編寫和輸入提示時,語言模型會產生更高質量的回應。通常在社會中向他人提出請求時,使用前者而不是後者的語氣時,對方更有可能認真執行所請求的任務,語言模型似乎也學習並模仿了人類的這種回應模式。
...
當要求條件複雜且有多個時,將每個項目列成清單並簡潔地傳達,比逐一詳細描述更易於閱讀和理解(無論是對人還是語言模型)。此外,必要時提供示例也會有所幫助。
在這種情況下,我添加了以下條件:
...
然而,在翻譯文章時,雖然標題(title)和描述(description)標籤需要翻譯成多種語言,但為了保持文章URL的一致性,最好將類別(categories)和標籤(tags)名稱保留為英文原文。因此,我添加了以下指示,以確保除了'title'和'description'之外的標籤不被翻譯。由於Claude應該已經學習並了解YAML front matter的相關信息,這樣的說明在大多數情況下應該足夠了。
...
> 我添加了"under any circumstances, regardless of the language you are translating to"這句話,以強調**無一例外地**不要隨意修改YAML front matter中的其他標籤。
{: .prompt-tip }
...
- 對於專業術語:
  ...
  - 翻譯成西班牙語、葡萄牙語、法語等羅馬字母基礎的語言時,允許單獨使用"翻譯表達"或同時使用"翻譯表達(英語表達)",讓Claude自主選擇更合適的方式。
- 對於專有名詞,無論以何種形式,原文拼寫都必須以某種方式保留在翻譯結果中。
...
一些文章包含連接到其他文章的鏈接,在測試階段,如果沒有為此提供具體指示,模型會將URL的路徑部分也視為需要翻譯的對象,導致內部鏈接失效的問題經常發生。通過在提示中添加以下句子解決了這個問題:
...
最後,提出以下句子,要求回應時不要添加其他話語,只輸出翻譯結果:
...
對於需要相當程度推理的任務,如解決數學問題或撰寫複雜文檔,引導語言模型將問題分步思考可以大大提高性能。但這種情況下可能會增加響應延遲時間,而且並非所有任務都適用這種技巧,需要注意。
...
自從最初撰寫這篇文章以來,雖然中間經過一兩次小幅度的提示改進和指示具體化,但在4個月內應用這個自動化系統時並沒有遇到重大問題。

然而,從韓國時間12024.10.31.晚上6點左右開始,當委託翻譯新撰寫的文章時,持續出現只翻譯文章開頭'TL;DR'部分後就任意中斷翻譯的異常現象。
