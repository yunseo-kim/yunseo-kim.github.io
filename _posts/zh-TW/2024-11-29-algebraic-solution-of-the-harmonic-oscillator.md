...
## 諧振子的定態
### 定態$\psi_n$和能量本徵值$E_n$
如果我們不斷應用降階算符，最終會得到一個能量小於$0$的狀態，這在物理上是不可能的。數學上，如果$\psi$是薛丁格方程的解，那麼$\hat{a}_-\psi$也是解，但這個新解並不總是能被規範化（即不總是物理上可能的狀態）。不斷應用降階算符，最終我們會得到平凡解$\psi=0$。

因此，對於諧振子的定態$\psi$，存在一個"最低階"$\psi_0$，滿足

$$ \hat{a}_-\psi_0 = 0 \tag{19}$$

（不存在更低的能量本徵值）。這個$\psi_0$滿足
...
現在將這個解代入我們之前得到的薛丁格方程（$\ref{eqn:schrodinger_eqn_with_ladder}$），並利用$\hat{a}_-\psi_0=0$，我們得到：

$$ E_0 = \frac{1}{2}\hbar\omega \label{eqn:E_ground}\tag{20}$$

從這個**基態（ground state）**開始，不斷應用升階算符，每應用一次升階算符，能量就增加$\hbar\omega$，我們可以得到所有激發態（excited states）。

$$ \psi_n(x) = A_n(\hat{a}_+)^n \psi_0(x),\quad E_n = \left(n + \frac{1}{2} \right)\hbar\omega \label{eqn:psi_n_and_E_n}\tag{21}$$

這裡$A_n$是規範化常數。這樣，我們可以通過找到基態，然後應用升階算符來確定諧振子的所有定態和允許的能量本徵值。

### 規範化
規範化常數也可以用代數方法求得。我們知道$\hat{a}_{\pm}\psi_n$與$\psi_{n\pm 1}$成正比，所以我們可以寫成：
...
因此，如果我們令$f=\hat{a}_\pm \psi_n$，$g=\psi_n$，我們得到：
...
然後，從方程（$\ref{eqn:schrodinger_eqn_with_ladder}$）和（$\ref{eqn:psi_n_and_E_n}$），我們有：
